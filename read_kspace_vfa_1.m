function [A, kr_arr, coil_sens, im_k_arr] = read_kspace_vfa_1(fa,num_proj,DATA)
...
%    idx_i_tr = floor(i_tr/4) + 1;
   ...
   k{idx_i_tr} = kx + 1i*ky;
   kr_arr{idx_i_tr} = kr;
   im_k_arr{idx_i_tr} = im_k;
   
   
   im_k = [];
   for i_coil = 1:num_coil
      [kx,ky,kr,im_k(:,i_coil),~] = read_kspace_rad_vfa_vd_coil(fa(i_tr+1),i_coil,num_proj,DATA);
   end
   k{idx_i_tr} = [k{idx_i_tr};kx + 1i*ky];
   kr_arr{idx_i_tr} = [kr_arr{idx_i_tr};kr];
   im_k_arr{idx_i_tr} = [im_k_arr{idx_i_tr};im_k];
   
   
   im_k = [];
   for i_coil = 1:num_coil
      [kx,ky,kr,im_k(:,i_coil),~] = read_kspace_rad_vfa_vd_coil(fa(i_tr+2),i_coil,num_proj,DATA);
   end
   k{idx_i_tr} = [k{idx_i_tr};kx + 1i*ky];
   kr_arr{idx_i_tr} = [kr_arr{idx_i_tr};kr];
   im_k_arr{idx_i_tr} = [im_k_arr{idx_i_tr};im_k];
   ...
   im_k = [];
   for i_coil = 1:num_coil
      [kx,ky,kr,im_k(:,i_coil),~] = read_kspace_rad_vfa_vd_coil(fa(i_tr+3),i_coil,num_proj,DATA);
   end
   k{idx_i_tr} = [k{idx_i_tr};kx + 1i*ky];
   kr_arr{idx_i_tr} = [kr_arr{idx_i_tr};kr];
   im_k_arr{idx_i_tr} = [im_k_arr{idx_i_tr};im_k];
   ...
   A{idx_i_tr} = NUFFT(k{idx_i_tr}, 1, 1, 0, [256,256], 2);
